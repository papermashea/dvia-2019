...
function sidebar() {
  // These commands are applied to the graphics canvas as normal.
    ...
    fill(0,0,0)
    textSize(16)
    text('Totals of hospitals', 40, 360)
    // text('Totals of ${hosTable.getRowCount()} hospitals', 40, 360)
    ...
    fill(0,0,0)
    textSize(21)
    text(`Monthly Vulnerable Facilities`, 40, 520)

    fill(0,0,0);
    textSize(16);
    // circle(hosFacility * 5);
    text('Week 1', 40, 560);
    text('Week 2', 40, 580);
    text('Week 3', 40, 600);
    text('Week 4', 40, 620);
...
function addvulnerableFacilities(){
        ...
        L.circleMarker([quake.getNum('latitude'), quake.getNum('longitude')], {
            ...
            fillOpacity:1,
            radius:10
        }).bindTooltip(quake.getString('place')).addTo(mymap)
        ...
        for (var r=0; r<watTable.getRowCount(); r++){
            ...
            if (disancetInKm < 50){
                L.circleMarker([treatment.getNum('latitude'), treatment.getNum('longitude')], {
                    ...
                    fillOpacity:1,
                    radius:4
                }).bindTooltip(treatment.getString('STATE_WATER_BODY_NAME')).addTo(mymap)
            }
        }  
...
//     var hosCount = addvulnerableFacilities();
//     for (var i=0; i<hosTable.getRowCount(); r++){
//         var hosFacilities = hosTable.getRow(r)

//         // count total if it is near quake
//         var disancetInKm = distanceFrom(quake.getNum('latitude'), quake.getNum('longitude'), treatment.getNum('latitude'), treatment.getNum('longitude'))
//         if (disancetInKm < 50){
//             count++;} return total
//     }
